...
```yaml
- cron: "0 2,4,6,8 * * 1-5"   # every 2h during the Jakarta session
- cron: "0 12 * * *"          # full-history run after the close
```

The intraday runs fetch a one-month window and merge into the existing file, so
long history is never truncated. Frequent snapshots are what make the in-app
Refresh reliable: it re-reads a committed file rather than depending on a
public relay being up.
...
**Refresh says the live feed didn't answer.**
Expected, and harmless. Browsers can't call Yahoo directly, so the live attempt
goes through public CORS relays that rate-limit without warning. The app falls
back to the snapshot committed in this repository — same-origin, so it can't be
blocked — and every figure stays correct. Only the intraday price is missing.
Press **Check** on the Portfolio tab to test each source separately.

**Adding a ticker looks like it failed.**
It didn't. The holding is saved the moment you press Add; the price fetch is a
separate step that's allowed to fail. Use **Look up ticker** in the dialog first
— it fills in the name and currency and tells you plainly whether Yahoo doesn't
recognise the symbol (fix the ticker) or simply couldn't be reached (wait and
refresh). A bare `BBRI` is caught offline, before any network call.
